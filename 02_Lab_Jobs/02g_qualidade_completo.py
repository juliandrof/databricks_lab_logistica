...
# MAGIC %md
# MAGIC # Lab 2c - Verificacao de Qualidade dos Dados (COMPLETO)
# MAGIC
...
# Check 5a: Verificar que a tabela gold_volume_por_rota possui dados
try:
    total_rotas = spark.sql(f"""
        SELECT COUNT(*) AS total
        FROM {catalog_name}.gold.gold_volume_por_rota
    """).collect()[0]["total"]
    ...
    print(f"⚠️ Erro ao verificar gold_volume_por_rota: {e}")

# Check 5b: Verificar que nao ha total_pedidos negativo
try:
    ...
    resultados_qualidade.append({
        "check": "total_pedidos negativo (gold_volume_por_rota)",
        "resultado": int(pedidos_negativos),
        "esperado": "0 registros negativos",
        "status": status,
    })
    print(f"{'✅' if status == 'OK' else '❌'} Registros com total_pedidos negativo: {pedidos_negativos}")
    ...
    print(f"⚠️ Erro ao verificar total_pedidos: {e}")

# Check 5c: Verificar que nao ha peso_total negativo
try:
    ...
    resultados_qualidade.append({
        "check": "peso_total negativo (gold_volume_por_rota)",
        "resultado": int(peso_negativo),
        "esperado": "0 registros negativos",
        "status": status,
    })
    print(f"{'✅' if status == 'OK' else '❌'} Registros com peso_total negativo: {peso_negativo}")
    ...
    print(f"⚠️ Erro ao verificar peso_total: {e}")
